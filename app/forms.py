...
class SearchPatientForm(FlaskForm):
    ...
    min_age = IntegerField("Age", validators=[Optional(), NumberRange(min=0, max=120)])
    max_age = IntegerField("   ", validators=[Optional(), NumberRange(min=0, max=120)])
...
class CreateAppointmentForm(FlaskForm):
    ...
    def validate_date(self, field):
        ...
        if field.data >= dt.date.today() + dt.timedelta(days=365):
            raise ValidationError("Appointments can only be booked at most a year ahead.")
